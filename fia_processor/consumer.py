...
LOGIN_ADMIN = "Kate"
PASS_USER = "mary_pass"
PASS_ADMIN = "kate_pass"

def check_new_data(data: list, details):
    details["auths"] = []
    fail_auth = 0
    if details['login'] == LOGIN_USER:
        if details['password'] == PASS_USER:
            details["auths"].append(
            {
            ...
            fail_auth = 1
    elif details['login'] == LOGIN_ADMIN:
        if details['password'] == PASS_ADMIN:
            details["auths"].append(
            {
                "source_id": details["id"],
...
def handle_event(id: str, details: dict):    
    ...
    try:
        if details['operation'] == 'process_new_users_request':
            new_data = details['new_data']
            check_new_data(new_data, details)
            if len(details['auths']) > 0:
                details['deliver_to'] = 'fdp_output'
                details['operation'] = 'process_new_auth'
